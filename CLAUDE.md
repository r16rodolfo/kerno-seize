...
Este projeto segue o **Kerno Seize v6.0.2** — um framework de desenvolvimento completo que integra PM Skills (decisão de produto), BMAD Method (planejamento técnico), WDS (design), e Kerno (governança, segurança e documentação). Implementação via Claude Code direto ou Lovable.
...
1. Incremente o patch version (ex: 6.0.2 → 6.0.2) no arquivo modificado
