...
Este projeto segue o **Kerno Seize v6.1.1** — um framework de desenvolvimento completo que integra PM Skills (decisão de produto), BMAD Method (planejamento técnico), WDS (design), e Kerno (governança, segurança e documentação). Implementação via Claude Code direto ou Lovable.
...
1. Incremente o patch version (ex: 6.1.1 → 6.1.1) no arquivo modificado
